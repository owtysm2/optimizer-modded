> [!CAUTION]
> **In development.** Much of the lesser-used menus are not yet updated.

> [!CAUTION]
> ### This is a fork.<br>
> No support will be provided.<br>
> Use this modified version at your own risk.

> No new features will be added.

# Optimizer Modded
An opinionated UI refresh of your favourite Windows optimization tool, with the addition of a UI for pre-made presets.
<p align="center">
	<img src="https://i.imgur.com/Rmx37V3.png">
	<a href="https://github.com/owtysm2/optimizer-modded/releases/latest" target="_blank">
		<img src="https://i.imgur.com/deiuL4g.png">
	</a>
</p>

---

<center>
<h2>💾 Downloads</h2> 
</center>

Find the latest release of Optimizer Modded on the [Releases](https://github.com/owtysm2/optimizer-modded/releases) page.

| Version | Download Link |
| ------- | ------------- |
| Latest | [16.7 (Modded, Dev)](https://github.com/owtysm2/optimizer-modded/releases/latest) |

<center>
<h2>🌍 Showcase</h2> 
</center>

<p align="center">
   <img src="https://i.imgur.com/rP7HraB.png">
</p>

<center>
<h2>🤖 Templating and Automation</h2> 
</center>

Explore the possibilities of automation with Optimizer through our [Automation Guide <img src="https://i.imgur.com/9cwvXdg.png">](https://github.com/owtysm2/optimizer-modded/blob/master/AUTOMATION.md).

<center>
<h2>⌨️ Command-line Options</h2> 
</center>

Check out the [Command-line Options <img src="https://i.imgur.com/9cwvXdg.png">](https://github.com/owtysm2/optimizer-modded/blob/master/CONFS.md) available for Optimizer.

<center>
<h2>❓ Frequently Asked Questions</h2> 
</center>

Find answers to common queries in the [FAQ Section <img src="https://i.imgur.com/9cwvXdg.png">](https://github.com/owtysm2/optimizer-modded/blob/master/FAQ.md).

<center>
<h2>💻 Compatibility</h2> 
</center>

- Requires .NET Framework 4.7.2
- Compatible with Windows 7, 8, 8.1, 10, 11
- Can run on Windows Server 2008, 2012, 2016, 2019, 2022 using `/unsafe` switch

<center>
<h2> ☕ Buy the original creator a delicious espresso</h2>
</center>

If you find this tool useful, consider showing your support by [donating through PayPal <img src="https://i.imgur.com/9cwvXdg.png">](https://www.paypal.com/paypalme/supportoptimizer).

<center>
<h2>❤️ Credits and Acknowledgments</h2>
</center>

- <img src="https://i.imgur.com/q2pYhSa.png"> [Optimizer](https://github.com/hellzerg/optimizer) - The original Optimizer project, maintained by deadmoon ([GitHub Profile](https://github.com/hellzerg))
- <img src="https://i.imgur.com/WoCadWf.png"> [ByteSize](https://github.com/omar/ByteSize) - A useful byte conversion library by Omar Rahman ([GitHub Profile](https://github.com/omar))
- <img src="https://i.imgur.com/K8swxaE.png"> [ColorPicker](https://github.com/search?q=repo%3Aowtysm2%2Foptimizer-modded+ColorPicker&type=commits) - Theme engine, courtesy of cat ([GitHub Profile](https://github.com/vadiscode))

`I might've replaced the ColorPicker entirely with the CuoreUI cuiColorPicker, but I will keep the credit here for their past work on this amazing project.`

<center>
<h2>❤️ Translations</h2>
</center>

These are the translations which were present when this fork had been made!
We'd like to extend our gratitude to the following contributors for their translations:

| Language   | Contributor(s)             |
| ---------- | -------------------------- |
| <img src="https://i.imgur.com/lVU0qfr.png"> Arabic     | MesterPerfect              |
| <img src="https://i.imgur.com/djWpngg.png"> Bulgarian  | `*`1Kill2Steal               |
| <img src="https://i.imgur.com/9MN5Z61.png"> Chinese    | btwise                     |
| <img src="https://i.imgur.com/efq2VtY.png"> Croatian   | zZan54                     |
| <img src="https://i.imgur.com/IJsDQdF.png"> Czech      | Tom Longhorn               |
| <img src="https://i.imgur.com/dON2OyN.png"> Dutch      | svanlaere                  |
| <img src="https://i.imgur.com/7gSiyW5.jpeg"> Farsi     | MjavadH                    |
| <img src="https://i.imgur.com/rDpgqRe.png"> French     | RAFF47                     |
| <img src="https://i.imgur.com/YgdinEE.png"> German     | theflamehd                 |
| <img src="https://i.imgur.com/jmw1YoK.png"> Hellenic   | `*`aplenaki                  |
| <img src="https://i.imgur.com/NFG6Gd6.png"> Hungarian  | Zan                        |
| <img src="https://i.imgur.com/TSaUt7h.png"> Indonesian | ftrsndrya                  |
| <img src="https://i.imgur.com/0WViCMM.png"> Italian    | Ziocash                    |
| <img src="https://i.imgur.com/Vb7Y1oH.png"> Japanese   | Yamada Hayao, creeper-0910 |
| <img src="https://i.imgur.com/1BR6dDr.png"> Korean     | VenusGirl                  |
| <img src="https://i.imgur.com/Op9ufzW.png"> Kurdish    | Parwar Andam               |
| <img src="https://i.imgur.com/c50dnsY.png"> Nepali     | chapagetti                 |
| <img src="https://i.imgur.com/pg4jRh8.png"> Polish     | Wilamaxin                  |
| <img src="https://i.imgur.com/seAcPLV.png"> Portuguese | Cassio                     |
| <img src="https://i.imgur.com/hXobAIi.png"> Romanian   | BeamingNG, DefaultUser9148 |
| <img src="https://i.imgur.com/mC6tVpf.png"> Russian    | mrkaban                    |
| <img src="https://i.imgur.com/ABXXMJf.png"> Spanish    | danielcshn                 |
| <img src="https://i.imgur.com/XHdXyZU.png"> Taiwanese  | H3XDaemon                  |
| <img src="https://i.imgur.com/umU3r09.png"> Turkish    | Kheasyque                  |
| <img src="https://i.imgur.com/g2BB7Dd.png"> Ukrainian  | Kirill Ermakov             |
| <img src="https://i.imgur.com/nCwacUp.png"> Urdu       | `*`shoaib-jamal              |
| <img src="https://i.imgur.com/FlAoyUk.png"> Vietnamese | `*`michioxd                  |


`* - Translations with the asterisk didn't have a contributor provided in the original readme, but I went through the commits and attributed who I THINK had contributed. If there are any errors, please correct me!`

`Also, some attributions weren't present in the original hellzerg/optimizer repo at the time that I had forked this repo, so I added the respective thanks (also based on what I could read from the commit history)`

---

many sins have been committed in this codebase
